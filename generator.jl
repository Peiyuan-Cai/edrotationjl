...
function hamiltonian(hlist, L, paras=Dict())
    ...
    for k in 1:3
        ...
        for l = 1:length(srtlst[k])
            # compass
            ham += Jc*gamma * threeOP([sprsp, sprsp], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
            ham += Jc*conj(gamma) * threeOP([sprsm, sprsm], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
            # in-out
            ham += -1im*Jcz*conj(gamma)/2 * threeOP([sprsp, sprsz], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
            ham += 1im*Jcz*gamma/2 * threeOP([sprsm, sprsz], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
            ham += -1im*Jcz*conj(gamma)/2 * threeOP([sprsz, sprsp], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
            ham += 1im*Jcz*gamma/2 * threeOP([sprsz, sprsm], [Int(srtlst[k][l][1]), Int(srtlst[k][l][2])], L)
        end
    end
...
function main()
    ...
    hl = Hlist(bcon, Lx, Ly)
    hm = hamiltonian(hl, L, Dict("Jxy"=>args["Jxy"], "Jz"=>args["Jz"], "Jc"=>args["Jc"], "Jcz"=>args["Jcz"]))
